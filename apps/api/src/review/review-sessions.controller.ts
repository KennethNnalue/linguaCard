...
import { ReviewSessionsService, UpsertSessionDto } from './review-sessions.service';
...
export class ReviewSessionsController {
  ...
  @Post()
  upsert(@CurrentUser() userId: string, @Body() dto: UpsertSessionDto) {
    return this.sessionsService.upsert(userId, dto);
  }

  /** Batch upsert — called by the offline sync handler when reconnecting. */
  @Post('batch')
  upsertBatch(@CurrentUser() userId: string, @Body() sessions: UpsertSessionDto[]) {
    return this.sessionsService.upsertBatch(userId, sessions);
  }
