...
@Component({
  ...
  standalone: true,
  imports: [IonButton, IonContent, IonIcon, IonInput, IonItem, IonSelect, IonSelectOption, IonSpinner, IonTextarea, ReactiveFormsModule],
...
export class AdminPodcastTopicsPage implements OnInit {
  ...
  readonly editTopicForm = new FormGroup({
  ...
  readonly detailsForm = new FormGroup({
    title: new FormControl('', {
      nonNullable: true,
      validators: Validators.required
    }), translation: new FormControl('', {nonNullable: true}), description: new FormControl('', {nonNullable: true})
  });
  ...
  constructor() {
    ...
    effect(() => {
      const id = this.store.lastCreatedEpisodeId(), payload = this.pendingTranscript();
      if (id && payload) untracked(() => {
        this.store.previewTranscript({episodeId: id, payload});
    ...
    effect(() => {
      ...
      if (id && id === this.topicId()) untracked(() => void this.router.navigate(['/admin/podcasts']));
    });
    effect(() => {
  ...
  createTopic(): void {
    if (this.topicForm.valid) this.store.createTopic(this.topicForm.getRawValue());
  }

  updateTopic(): void {
    const topic = this.topic();
    if (topic && this.editTopicForm.valid) this.store.updateTopic({
      topicId: topic.id,
      dto: this.editTopicForm.getRawValue()
    });
  }

  updateEpisode(): void {
    const episode = this.episode();
    if (episode && this.detailsForm.valid) {
      const value = this.detailsForm.getRawValue();
      this.store.updateEpisode({
        episodeId: episode.id,
        dto: {title: value.title, titleTranslation: value.translation, description: value.description}
      });
    }
  }
  ...
  openTopic(id: string): void {
    void this.router.navigate(['/admin/podcasts', id]);
  }

  openEpisode(topicId: string | null, episodeId: string | null): void {
  ...
  reviewTranscript(): void {
    ...
    if (this.store.transcriptDetails()?.episodeId !== episode.id) this.store.loadTranscript(episode.id);
  }
  ...
  openTranscriptWorkspace(): void {
    const episode = this.episode();
    if (episode && episode.status !== 'published') void this.router.navigate(['/admin/podcasts', episode.topicId, 'episodes', episode.id, 'transcript']);
  }
  ...
  publishTopic(): void {
    const topic = this.topic();
    if (topic) this.store.publishTopic(topic.id);
  }

  hasPublishedEpisode(): boolean {
    return this.topic()?.episodes.some(episode => episode.status === 'published') ?? false;
  }

  async deleteTopic(): Promise<void> {
    const topic = this.topic();
    if (!topic || !await this.confirm(`Delete “${topic.title}” and all of its episodes?`)) return;
    this.store.deleteTopic(topic.id);
  }

  async deleteEpisode(): Promise<void> {
    const episode = this.episode();
    if (!episode || !await this.confirm(`Delete “${episode.title}”?`)) return;
    this.store.deleteEpisode(episode.id);
  }

  uploadTopicThumbnail(event: Event): void {
    const topic = this.topic(), file = this.selectedFile(event);
    if (topic && file) this.store.uploadTopicThumbnail({
      topicId: topic.id,
      upload: {file, accessibilityDescription: `${topic.title} cover artwork`, focalPointX: .5, focalPointY: .5}
    });
  }

  uploadEpisodeThumbnail(event: Event): void {
    const episode = this.episode(), file = this.selectedFile(event);
    if (episode && file) this.store.uploadEpisodeThumbnail({
      episodeId: episode.id,
      upload: {file, accessibilityDescription: `${episode.title} artwork`, focalPointX: .5, focalPointY: .5}
    });
  }
  ...
  speakerName(key: string): string {
    return this.store.transcriptDetails()?.speakers.find(speaker => speaker.key === key)?.name ?? key;
  }
