...
## Replace Gemini TTS (3 RPM bottleneck) with Google Cloud TTS WaveNet
...
- **Model:** `de-DE-Wavenet-B` (German, male, WaveNet quality)
- **Free tier:** **4 million characters/month**, no RPM cap, no request rate limit
- **SDK:** `@google-cloud/text-to-speech` — separate from AI Studio SDK
- **Auth:** Service account JSON (different key from `GEMINI_API_KEY`)

> **Why WaveNet over Neural2?** Your GCP console screenshots confirm the current pricing structure. WaveNet voices have a 4M chars/month free tier (vs Neural2's 1M) and cost $4/1M after the free tier (vs Neural2's $16/1M). WaveNet quality is significantly better than Gemini TTS for German and fully sufficient for a language learning app. Neural2 is available as an upgrade path if you determine voice quality needs improvement after launch.

> **Important: Gemini-family TTS inside Cloud TTS API is NOT free.** Your screenshots show that `Gemini 2.5 Flash TTS`, `Gemini 2.5 Pro TTS`, and `Gemini 3 Flash TTS` within the Cloud TTS API are token-based and have **no free tier**. The adapter must explicitly use a WaveNet voice name — never a Gemini-family model — to stay within the free tier.

> **Critical first step:** Your GCP console shows the **Enable** button for the Cloud Text-to-Speech API on the `linguacard-production` project, meaning the API is not yet enabled. Enabling it is Step 1 before anything else will work.
...
### Step 1 — Enable the Text-to-Speech API (do this first — it is not yet enabled)

> **Your GCP console confirms the Cloud Text-to-Speech API is not yet enabled** on the `linguacard-production` project — the **Enable** button is visible on the product page. This is the very first action. Nothing else works until this is done.

1. Go directly to: **https://console.cloud.google.com/apis/library/texttospeech.googleapis.com?project=linguacard-production**
   (this is the exact URL you already had open)
2. Click the **Enable** button (top-right of the page)
3. Wait for enablement to complete — usually under 30 seconds
4. The page will refresh and show **API enabled** with usage graphs
...
### Step 4 — Prepare the key for environment variables
...
GOOGLE_CLOUD_TTS_VOICE=de-DE-Wavenet-B
GOOGLE_CLOUD_TTS_LANGUAGE=de-DE
```

### Step 6 — Add to Render (production)
...
### Available German voices — voice selection guide

> **Pricing reality check from your GCP console:** The pricing page lists separate billing SKUs for each voice tier. WaveNet and Standard voices share a 4M char/month free tier. Neural2 has 1M/month free. Gemini-family voices inside Cloud TTS API have **no free tier at all** — do not use them.

| Voice ID | Gender | Free tier | After free | Recommendation |
|---|---|---|---|---|
| `de-DE-Wavenet-B` | Male | **4M chars/month** | $4/1M | ✅ **Default — use this** |
| `de-DE-Wavenet-A` | Female | 4M chars/month | $4/1M | Alternative if female voice preferred |
| `de-DE-Wavenet-C` | Female | 4M chars/month | $4/1M | Warmer female tone |
| `de-DE-Wavenet-D` | Male | 4M chars/month | $4/1M | Slightly younger-sounding male |
| `de-DE-Wavenet-F` | Female | 4M chars/month | $4/1M | High clarity female |
| `de-DE-Neural2-B` | Male | 1M chars/month | $16/1M | Upgrade path if WaveNet quality is insufficient |
| `de-DE-Neural2-A` | Female | 1M chars/month | $16/1M | Neural2 female upgrade path |
| `de-DE-Standard-B` | Male | 4M chars/month | $4/1M | Lower quality — avoid |
| Gemini 2.5 Flash TTS | — | **None** | ~$0.50/1M audio tokens | ❌ This is what we're replacing |
...
export interface AiConfig {
  ...
  googleCloudTtsVoice:     string;   // ← new: default 'de-DE-Wavenet-B'
...
# Free tier: WaveNet voices = 4M chars/month (no expiry). DO NOT use Gemini-TTS
# voice names here — they have no free tier and are billed from character 0.
...
export class GoogleCloudTTSAdapter implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    // Guard: Gemini-family voice names inside Cloud TTS API are NOT free-tier
    // and will generate charges from the first character. Refuse to start with them.
...
- [ ] `onModuleInit()` logs an error and sets `isConfigured=false` if voice name contains `'gemini'` — prevents accidental paid Gemini-TTS usage
...
- [ ] Manual test: `generateSpeech({ text: 'der Hund' })` produces a playable MP3 with voice `de-DE-Wavenet-B`
...
The 4 million free characters/month on WaveNet is extremely generous — a story narration is ~2,000 characters, a word ~10–15 characters. At 100 active users, typical monthly usage would be ~50,000–100,000 characters for a normal app (well within the free tier). The fallback is defensive insurance, not an expected daily path.
...
private async generateSpeechWithFallback(
  ...
  // Primary: Google Cloud TTS (no RPM limit, 4M WaveNet chars/month free)
  try {
...
describe('GoogleCloudTTSAdapter', () => {
  ...
  describe('onModuleInit()', () => {
    ...
    it('logs error and sets isConfigured=false when voice name contains "gemini" — prevents free-tier bypass');
  });
...
## Rate Limit & Cost Comparison: Before vs After

| Provider | Use | Before | After |
|---|---|---|---|
| Gemini TTS (`gemini-2.5-flash-preview-tts`) | All TTS | 3 RPM, **no free tier on Cloud TTS API** | Fallback only |
| Google Cloud TTS (`de-DE-Wavenet-B`) | All TTS | Not in use | Primary — **no RPM cap**, **4M chars/month free** |
| Google Cloud TTS Neural2 (`de-DE-Neural2-B`) | — | Not in use | Optional upgrade — 1M chars/month free, $16/1M after |
| Gemini-family voices in Cloud TTS API | — | N/A | ❌ Never use — no free tier, billed from character 0 |
...
After this epic: zero paid API calls for audio in a normal month under 4M characters. Zero RPM limits blocking pronunciation or story playback.

---

## Improvements to Existing Audio Service

Beyond the provider swap, this epic introduces three improvements worth noting:

**1. 4× more free quota and 75% lower cost if exceeded.** WaveNet gives 4M characters/month free (vs Neural2's 1M). At $4/1M after the free tier (vs $16/1M for Neural2), if LinguaCard ever grows beyond the free tier the cost differential is significant. This is a better default for an app at early scale — Neural2 remains available via env var if voice quality needs to be upgraded.

**2. SSML pronunciation quality (LC-120)** — Gemini TTS had no SSML support at all. Google Cloud TTS fully supports it. The `SsmlBuilder` adds controlled speaking rate (0.85× for single words, 0.92× for stories) which is meaningful for language learners who need to catch individual phonemes. This is a net new capability that did not exist before.

**3. Gemini voice guard in `onModuleInit()`** — The GCP console shows that Gemini-family TTS voices within the Cloud TTS API (Gemini 2.5 Flash TTS, Gemini 2.5 Pro TTS, Gemini 3 Flash TTS) have **no free tier**. They are billed from the first character. The adapter now actively refuses to start if configured with a Gemini voice name, logging a clear error and staying unconfigured rather than silently generating unexpected bills. This guard is permanent defensive infrastructure.
